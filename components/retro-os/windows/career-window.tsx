...
export function CareerWindow() {
  ...
  return (
    <div className="flex h-full text-[16px] text-black">
      {/* Left panel - Job list (tree view style) */}
      <div className="w-[270px] shrink-0 border-r-2 border-[#808080] bg-white p-1.5 overflow-y-auto win95-scrollbar">
        <div className="text-[15px] font-bold text-[#808080] px-1.5 mb-1.5">
          {t("career.timeline")}
        </div>
        {JOBS.map((job) => (
          <button
          ...
          >
            <span className="text-[15px] shrink-0 mt-px">
              {selectedJob === job.id ? "[-]" : "[+]"}
            </span>
            <div>
              <div className="font-bold text-[15px] leading-tight">{job.company[lang]}</div>
              <div className={`text-[14px] ${selectedJob === job.id ? "text-[#87CEEB]" : "text-[#808080]"}`}>
                {job.period[lang]}
              </div>
            </div>
          </button>
      ...
      <div className="flex-1 p-4 overflow-y-auto win95-scrollbar">
        {activeJob && (
          <div>
            <div className="mb-4 pb-3 border-b border-[#c0c0c0]">
              <h2 className="text-[21px] font-bold text-[#000080]">
                {activeJob.company[lang]}
              </h2>
              <p className="text-[18px] font-bold">{activeJob.role[lang]}</p>
              <p className="text-[15px] text-[#808080]">{activeJob.period[lang]}</p>
            </div>

            <p className="text-[16px] mb-4 p-3 bg-[#FFFFCC] win95-inset">
              {activeJob.description[lang]}
            </p>

            <p className="font-bold text-[16px] mb-1.5 text-[#000080]">
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {activeJob.tags.map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 bg-[#000080] text-white text-[14px]"
                >
                  {tag}
                </span>
              ))}
